...
const LAZY_FN_NAME = "ssrLazy"; // lazy
const LAZY_FN_SOURCE = "@/src/libs/ssr_lazy"; // solid-js
  ...
  return {
    ...
    visitor: {
      CallExpression(nodePath, state) {
        ...
        if (node.callee.type !== "Identifier" || node.callee.name !== LAZY_FN_NAME) return;

        const binding = nodePath.scope.getBinding(LAZY_FN_NAME);
        ...
        if (source !== LAZY_FN_SOURCE) return;
