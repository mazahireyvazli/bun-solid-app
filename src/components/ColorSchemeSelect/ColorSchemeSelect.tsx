...
const Styles = await ssrLazy(async () => {
  const colorSchemeSelectCSS = await import("@/src/components/ColorSchemeSelect/color-scheme-select.css?inline").then(
    (m) => m.default,
  );

  return {
    default: () => <style textContent={colorSchemeSelectCSS} />,
  };
});
